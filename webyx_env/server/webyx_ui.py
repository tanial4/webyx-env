...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .rem-counts { display: flex; gap: 6px; padding: 0 16px 12px; }
  .rem-pill {
    flex: 1;
    text-align: center;
    padding: 6px 4px;
  ...
  <div id="history-panel">
    ...
    <div id="history-table-wrap">
      <table id="history-table" style="width:100%;border-collapse:collapse;">
        <thead>
          <tr><th>Step</th><th>Action</th><th>Target</th><th>Fix</th><th>Reward</th><th>Event</th></tr>
        </thead>
...
function updateUI(obs, reward) {
  ...
  document.getElementById("s-score").textContent = obs.episode_score != null ? obs.episode_score.toFixed(3) : "—";
  document.getElementById("s-cumrew").textContent= obs.metadata?.cumulative_reward != null ? obs.metadata.cumulative_reward.toFixed(3) : "—";
  ...
  if (obs.done) {
    ...
    banner.textContent = "✓ DONE — Final score: " + (obs.episode_score?.toFixed(3) ?? "N/A");
